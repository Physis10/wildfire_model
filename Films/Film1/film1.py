...
# -*- coding: utf-8 -*-
"""
Created on Fri Dec  1 13:31:19 2023
...
### Dimensions (t,x,y)
ti,tf,nt = [0.0, 1.5, 20] # Time coordinate
xi,xf,nx = [-4, 4, 50] # x coordinate
yi,yf,ny = [-4, 4, 50] # y coordinate
...
if (point):
    ...
    frontx,fronty = [0.3*np.cos(theta)-0.5,0.2*np.sin(theta)] # Initial curve counter-clockwise parametrized
    out = True # If 'True', outward trajectories are computed. If 'False', inward ones are computed


### Parameters of the model
z = np.exp(-np.square(xx[0]-1)/2-np.square(yy[0]-1)/2)+0.5*np.exp(-np.square(xx[0]+1)/2-np.square(yy[0]+1)/2) # Surface
...
for n in range(steps+1):
    ...
    if not n == 0:
        solx = np.append(sol[:,0,n],[sol[0,0,n]],0) # Initial and final points of the curve are the same, in order to obtain a closed interpolation
        soly = np.append(sol[:,1,n],[sol[0,1,n]],0)
