...
# -*- coding: utf-8 -*-
"""
Created on Wed Mar  8 12:26:22 2023

@author: Enrique Pendás-Recondo

Geometric wildfire simulator using Rothermel's model, metric units
"""
...
H = 18608.47*np.ones([nt,nx,ny]) # Heat content (kJ/kg)
S_t = 0.0555*np.ones([nt,nx,ny]) # Total mineral content (kg minerals/kg wood)
S_e = 0.010*np.ones([nt,nx,ny]) # Effective mineral content ((kg minerals-kg silica)/kg wood)
...
z = 0.06*np.exp(-np.square(xx[0])/2-np.square(yy[0])/2) # Surface topography (height map, m)
dzx,dzy = np.gradient(z,x,y)
phi_s = np.arccos(np.divide(1,np.sqrt(1+np.square(dzx)+np.square(dzy)))) # Slope steepness (angle)
M_f = 0.5*np.ones([nt,nx,ny]) # Fuel moisture (kg moisture/kg wood)
U = 20*np.ones([nt,nx,ny]) # Midflame wind speed (m/min)
...
Gamma_max = np.power(0.0591+2.926*np.power(Sigma,-1.5),-1) # Maximum reaction velocity (min**-1)
Rho_b = np.divide(W_0,Delta) # Bulk density (kg/m**3)
Beta_op = 0.20395*np.power(Sigma,-0.8189) # Optimum packing ratio
Beta = np.divide(Rho_b,Rho_p) # Packing ratio
W_n = np.multiply(W_0,1-S_t) # Net fuel load (kg/m**2)
A = 8.9033*np.power(Sigma,-0.7913)
Gamma = np.multiply(np.multiply(Gamma_max,np.power(np.divide(Beta,Beta_op),A)),
                    np.exp(np.multiply(A,1-np.divide(Beta,Beta_op)))) # Reaction velocity (min**-1)
...
for i in range(nt):
    for j in range(nx):
        for k in range(ny):
            if Eta_s[i,j,k] > 1:
                Eta_s[i,j,k] = 1
I_r = Gamma*W_n*H*Eta_m*Eta_s # Reaction intensity (kJ/min/m**2)
Xi = np.multiply(np.power(192+7.9095*Sigma,-1),
                 np.exp(np.multiply(0.792+3.7597*np.sqrt(Sigma),Beta+0.1))) # Propagation flux ratio
C = 7.47*np.exp(-0.8711*np.power(Sigma,0.55))
B = 0.15988*np.power(Sigma,0.54)
E = 0.715*np.exp(-0.01094*Sigma)
Phi_w = np.multiply(C*np.power(3.281*U,B),np.power(np.divide(Beta,Beta_op),-E)) # Wind factor
Phi_s = np.multiply(5.275*np.power(Beta,-0.3),np.square(np.tan(phi_s))) # Slope factor
Epsilon = np.exp(np.divide(-4.528,Sigma)) # Effective heating number
Q_ig = 581+2594*M_f # Heat of preignition (kJ/kg)

R_0 = np.divide(I_r*Xi,Rho_b*Epsilon*Q_ig) # Maximum spread rate without wind and slope (m/min)
R = np.multiply(R_0,1+Phi_w+Phi_s) # Maximum spread rate (m/min)


### Geometric fire spread pattern
h = np.zeros([nt,nx,ny]) # Radius of the sphere (m/min)
for j in range(nx):
    for k in range(ny):
        if phi_s[j,k] > 0:
            h[:,j,k] = np.divide(R_0[:,j,k]*Phi_s[:,j,k],np.sin(phi_s[j,k]))
phi = phi_w # Orientation of the ellipse (angle)
a = R_0-h # Semi-major axis of the ellipse (m/min)
epsilon = np.divide(np.multiply(R_0,Phi_w),a) # Eccentricity of the ellipse
